...
Geocoder.setApiKey('AIzaSyC8iFoJf7Ta6A2l3ZCx0CZLKIqqVPaJOQ0');
...
export default class App extends Component {
  ...
  bookRide() {
    ...
    RNGooglePlacePicker.show((response) => {
      if (response.didCancel) {
        ...
        console.log('sending');
        var triggered = this.available_drivers_channel.trigger('client-driver-request', {
  ...
  _setCurrentLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      { enableHighAccuracy: false, timeout: 200000, maximumAge: 100000, distanceFilter: 10},
    );
  ...
  componentDidMount() {
    ...
    var pusher = new Pusher('029ae7d42952d804e948', {
      authEndpoint: 'https://ancient-peak-24645.herokuapp.com/pusher/auth',
      cluster: 'ap2',
      encrypted: true
    });
    ...
    this.available_drivers_channel.bind('pusher:subscription_error', function(status) {
  ...
  if(status == 408 || status == 503){
    ...
    this.user_ride_channel.bind('client-driver-message', (data) => {
      if(data.type == 'near_pickup'){
        ...
        console.log('meesage for near_pickup')
        this.setState({
          has_ridden: true
        });
      }
